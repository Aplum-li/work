  ...
  if(manager(19) || manager(20)){
	  ...
	  if ($type != 'add') {
	    if ($id) {
	      ...
	      $typeinfo = $db -> th_select($flink_type_table,$where,'*');
	      if (!$typeinfo) {
	        showmsg('获取分类信息出错');
	        exit();
	      }
                 ...
                 <form class="registerform" method="post" action="post.php">
                    <table width="100%" class="insert-tab">
                        <tr>
                            ...
                            <td style="width:280px;"><input type="text" value="<?php if(!empty($typeinfo)){echo $typeinfo['flink_type_name'];}?>" name="info[flink_type_name]" class="inputxt input400" datatype="*" nullmsg="请输入分类名称！" errormsg="请输入分类名称！" /></td>
                            <td>
