...
class Uploader
{
    ...
    private function upFile()
    {
        ...
        if (!(move_uploaded_file($file["tmp_name"], $this->filePath) && file_exists($this->filePath))) { //移动失败
        	...
        	@include_once $_SERVER["DOCUMENT_ROOT"].'/include/common.inc.php';
            if (@loadConfig('web_water')) {
              //如果是图片才加水印..
              $arr = array(".png", ".jpg", ".jpeg", ".gif", ".bmp");
              if(in_array($this->getFileExt(), $arr)){
                $this->imageWaterMark($this->filePath,9,ROOTPATH.'/'.loadConfig('web_mark'));
              }
